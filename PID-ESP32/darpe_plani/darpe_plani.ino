// ESP32-S3-N8R2 MAIN CONTROLLER CODE
// Receives commands via UART from ESP32-S3-N16R8 (PID_TOTAL)
// Controls 4-wheel drive robot with elevator and servo
// Runs on core 1
...
// --- UART ---
const int RX_PIN = 21;  // RX pin for UART from PID_TOTAL
const int TX_PIN = 10;  // TX pin for UART (if needed)
...
void setup() {
  ...
  // Initialize serial communication for UART from PID_TOTAL
  Serial.begin(115200, SERIAL_8N1, RX_PIN, TX_PIN);
  delay(500);

  Serial.println("ESP32-S3 N8R2 Baslatildi - UART Modu");
}
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    // No message received - stop all motors and handle continuous functions
...
void parseInput(String msg) {
  // Parse incoming UART messages in format: "AXIS/BUTTON id value"
